  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        {onBack && (
          <button onClick={onBack} className="text-sm text-slate-600 hover:text-ink">
            ← Back
          </button>
        )}
      </div>
      ...
      {kpis.length > 0 && (
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          {kpis.map((kpi, i) => (
            <div key={`${kpi.label}-${i}`} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-card">

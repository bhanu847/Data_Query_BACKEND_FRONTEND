  ...
  return (
    <div className="rounded-2xl border border-border bg-surface-1 shadow-card">
      <button onClick={() => setExpanded(!expanded)}
        className="flex w-full items-center justify-between px-4 py-3 text-sm font-semibold text-ink hover:bg-surface-1 rounded-t-2xl">
        <div className="flex items-center gap-2">
          <svg className="w-4 h-4 text-muted-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {expanded && (
        <div className="border-t border-border px-4 py-3 space-y-3 max-h-[400px] overflow-y-auto">
            ...
            <div className="flex items-center justify-between">
              <div className="flex flex-wrap gap-1.5">
                {Object.entries(filters).map(([col, val]) => (
                  <span key={col} className="inline-flex items-center gap-1 rounded-full bg-accent-violet/10 px-2.5 py-0.5 text-xs text-accent-violet">
                    {col}: {Array.isArray(val) ? `${val.length} selected` : typeof val === "object" ? `${val.min ?? ""}–${val.max ?? ""}` : String(val)}
                    <button onClick={() => {
                      const updated = { ...filters };
                      delete updated[col];
                      onFiltersChange(updated);
                    }} className="hover:text-accent-rose font-bold">×</button>
                  </span>
                ))}
              </div>
              <button onClick={clearAll} className="text-xs text-accent-rose hover:text-accent-rose shrink-0 ml-2">Clear all</button>
            </div>
          )}
          ...
          {categoricalCols.length > 0 && (
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-muted-2 mb-2">Categories</p>
          ...
          {numericCols.length > 0 && (
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-muted-2 mb-2">Numeric Ranges</p>
              {numericCols.slice(0, 5).map((col) => (
                <div key={col} className="mb-2">
                  <p className="text-xs font-medium text-muted mb-1">{col}</p>
                  <div className="flex gap-2">
                    <input type="number" placeholder="Min"
                      value={filters[col]?.min ?? ""}
                      onChange={(e) => setRangeFilter(col, e.target.value, filters[col]?.max)}
                      className="w-24 rounded-lg border border-border px-2 py-1 text-xs focus:border-brand/50 focus:outline-none" />
                    <span className="text-muted-2 text-xs self-center">to</span>
          ...
          {categoricalCols.length === 0 && numericCols.length === 0 && (
            <p className="text-xs text-muted-2 text-center py-2">No filterable columns detected</p>
          )}
        </div>
      )}
    </div>
  ...
  return (
    <div className="mb-1">
      <button onClick={toggle}
        className="flex w-full items-center justify-between rounded-lg px-2 py-1.5 text-xs text-muted hover:bg-surface-1">
      ...
      {open && (
        <div className="ml-2 mt-1 max-h-28 overflow-y-auto">
          {loading && <p className="text-xs text-muted-2 animate-pulse px-2">Loading...</p>}
          {config?.values?.map((v) => (
            <label key={v} className="flex items-center gap-2 px-2 py-0.5 text-xs text-muted hover:bg-surface-1 rounded cursor-pointer">
              <input type="checkbox" checked={selected.includes(v)} onChange={() => onToggle(v)}
                className="rounded border-border-2 text-brand focus:ring-brand" />
              {v}
            </label>
          ))}
        </div>
